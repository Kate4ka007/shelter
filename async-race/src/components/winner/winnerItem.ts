class WinnerItem {
  ...
  constructor(id: number, name: string, time: number, color: string, number: number) {
    this.id = id;
    this.name = name;
    if (!this.name) {
      this.name = 'Tesla';
    ...
    localStorage.setItem('winnerTime', time.toString());

    const best = JSON.parse(localStorage.getItem(`${this.name}-best`));
    this.theBest = Math.max(...best);
    if (this.theBest === -Infinity) {
      this.theBest = 10;
    }
